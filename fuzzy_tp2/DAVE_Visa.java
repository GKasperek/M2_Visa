...
public class DAVE_Visa implements PlugIn
{
  ...
  public void run(String arg){
    ...
    demande =JOptionPane.showInputDialog("Ratio : ");
    ...
    for(i = 0;i <nbclasses; i++){
      for(j = 0; j < nbpixels; j++){
        ...
        for(k = 0; k < kmax; k++){
          if (Dprev[k][j] != 0) {
            apparDegre_ij = apparDegre_ij + Math.pow(Dprev[i][j] / Dprev[k][j], 2/m -1);
          }else{
            apparDegre_ij += 1d;
          }
        }
        Uprev[i][j] = 1d / apparDegre_ij;
      }
    }
    ////////////////////////////////////////////////////////////
    ...
    while ((iter < itermax) && (stab > seuil))
    {
      ...
      for(k = 0; k < nbclasses; k++){
        ...
        for(i = 0; i < nbpixels; i++){
          redC   = redC + (Math.pow(Uprev[k][i], m) * red[i]);
          greenC = greenC + (Math.pow(Uprev[k][i], m) * green[i]);
          blueC  = blueC + (Math.pow(Uprev[k][i], m) * blue[i]);
      ...
      for(i = 0; i < nbclasses; i++){
        for(j = 0; j < nbpixels; j++){
          ...
          for(k = 0; k < kmax; k++){
            if(Dmat[i][j] != 0){
              u_ij += Math.pow(Dmat[i][j] / Dmat[k][j], 2.0/ (m - 1.0));
            }else{
              u_ij += 1d;
            }
          }
